...
int static iCounter_canwrapper;
...
//#define EBADF            9      /* Bad file number - can net not opened */
// timeout - GetMsg will return false after timeout periodCAN_ERR_MASK
bool CanWrapper::GetMsg(struct can_frame &frame, bool &extended, bool &rtr, bool &error, int &errorCode, struct timeval timeout)
{
    ...
    if(ret > 0)
    {
        ...
        //qDebug() << "from canwrapper :- bytesRead"<<bytesRead<<"id counter is"<<iCounter_canwrapper++;
